...
update :: proc(editor_player: ^Editor_Player_Data, dt: f32) {
	...
	editor_player.movement_speed = linalg.max(editor_player.movement_speed + rl.GetMouseWheelMove() * 15, 0) 

	editor_player.position +=
		((forward * input_snapshot.movement.y) +
			(right * input_snapshot.movement.x) +
			(linalg.cross(forward, right) * ((up ? 1 : 0) + (down ? -1: 0)))) * dt * editor_player.movement_speed
