...
//  Dev logic
    ...
    for (!glfw.WindowShouldClose(window) && should_run) {
        ...
        when CFG_DEV {
            gl.BeginQuery(gl.TIME_ELAPSED, queries[frame_index])
        } // start gpu timer
        ...
        when CFG_DEV {
            ...
            prev := (frame_index + 1) % 2
            available : u32
            gl.GetQueryObjectuiv(queries[prev], gl.QUERY_RESULT_AVAILABLE, &available)
            if available != 0 {
                gl.GetQueryObjectui64v(queries[prev], gl.QUERY_RESULT, &time_elapsed)
                fmt.printf("GPU Time: {:8.5f} ms\n", f64(time_elapsed) / 1e6) // ":.4f" is some absolute black fucking magic
            }
            frame_index = prev
        } // end gpu timer & print gpu timer result
    }
    cleanup()   
}
...
cleanup :: proc() {
    ...
    gl.DeleteBuffers(1, &VBO)   // probably not necessary, but doing it anyways cuz why not
    gl.DeleteVertexArrays(1, &VAO)
    gl.DeleteBuffers(1, &EBO)
    glfw.DestroyWindow(window)
    glfw.Terminate()
    fmt.println("Cleaned up yo mess 🗣️🗣️")
}